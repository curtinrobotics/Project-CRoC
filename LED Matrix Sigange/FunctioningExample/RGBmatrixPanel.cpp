...
void RGBmatrixPanel::updateDisplay(void) {
  ...
  *latport |= latmask; // Latch data loaded during *prior* interrupt
  ...
  if (++plane >= nPlanes) {   // Advance plane counter.  Maxed out?
    ...
    //C  ==== BK
    //D  ==== EN2
    *addrcport &= ~addrcmask;//BK LOW
    if(row%8 == 0){
      *addrbport |= addrbmask;//DIN HIGH
    }
    *addraport |= addramask;//CLK HIGH
    *addraport &= ~addramask;//CLK LOW
    *addrbport &= ~addrbmask;//DIN LOW
